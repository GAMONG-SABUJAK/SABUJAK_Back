...
import com.sabujak.gamong.exception.NotAllowedFileTypeException;
import com.sabujak.gamong.exception.S3PresignedException;
...
public class S3Service {
    ...
    public PresignedUrlRes issuePresignedAndCdnUrl(ReqPresignedUrl reqPresignedUrl) {
        if (!AllowedFileType.isAllowedMimeType(reqPresignedUrl.mimeType())) {
            throw new IllegalArgumentException("허용되지 않은 MIME 타입입니다.");
        }

        // 확장자 추출
        String ext = Optional.ofNullable(reqPresignedUrl.fileName())
                .filter(f -> f.contains("."))
        ...
        URL presignedUrl = generatePresignedPutUrl(key, reqPresignedUrl.mimeType());
